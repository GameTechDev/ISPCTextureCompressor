...
HRESULT InitFont11( ID3D11Device* pd3d11Device, ID3D11InputLayout* pInputLayout )
{
    ...
    {
		HRSRC hresinfo = FindResource(NULL,L"FONT",L"DDS");
		if(hresinfo)
		{
			unsigned long size = SizeofResource(NULL,hresinfo);
			HGLOBAL hg=LoadResource(NULL,hresinfo);
			char *ptr = (char*) LockResource(hg);
			hr = D3DX11CreateShaderResourceViewFromMemory( pd3d11Device, ptr,size, NULL, NULL, &g_pFont11, NULL);
		}	
		if(!hresinfo || hr!=S_OK)
		{
			V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"UI\\Font.dds" ) );
